...
int PrepareStoppingTablesdpass(){
  /* Read stopping data from file and fill arrays etc. */
  /* material version */
  int i,j,k,l,Zc;
  int result;
  ...
  char DpassFileName[1000];
  sprintf(DpassFileName, "%s/DPASS_DB.dat", DirectoryData);
  FILE *f = fopen(DpassFileName, "rb");
  //  FILE *f = fopen("./DPASS_DB.dat", "rb");
  ...
  fseek(f, 25, SEEK_SET);
  fread(EnerData.c, 1, 800, f);

  for (int i = 0; i < MAX_STOPPING_ENTRIES_dpass; i++){
    Enerdpass[i] = 1e6*EnerData.d[i]; // MeV/u to eV/u (the *u factor will; be dealt with in transport.c
  ...
  /* Go through materials and create stopping tables for all existing elements */
  for(i=0;i<NumberOfMaterials;i++){
    ListOfMaterials[i].StoppingZEdpass=(float**)malloc(sizeof(float*)*MAX_ELEMENT_NO); /* pointers to stopping arrays */
    if(ListOfMaterials[i].StoppingZEdpass==NULL){return -1;}
    /* Go through possible elements which might be slowed down in this materials */
    for(j=0;j<=ZMAXdpass;j++){ /* Goes through all possible projectiles */
      if(existing_elements[j]==1){ /* ok, element might occur, calculate */
	/* Allocate memory for stopping table */
	ListOfMaterials[i].StoppingZEdpass[j]=(float*)calloc(MAX_STOPPING_ENTRIES_dpass,sizeof(float));
	if(ListOfMaterials[i].StoppingZEdpass[j]==NULL){return -1;}
	////////////////////////////////////////////////////////////////////////
	
	for(k=0;k<ListOfMaterials[i].ElementCount;k++){ /* Go through elements of target material */
	  Zc=ListOfMaterials[i].ElementsZ[k];
	  /*	  printf("projZ cible Z %d %d ",j,Zc);*/
	  long long pos = 828LL + (long long)(j - 1) * 803 * ZMAXdpass + (Zc - 1) * 803;
	  fseek(f, pos, SEEK_SET);
	  fread(dEdX.c, 1, 800, f);
	  for (int ie = 0; ie < MAX_STOPPING_ENTRIES_dpass; ie++){
	    energ=Enerdpass[ie];
	    sep=(dEdX.d[ie]*1e-15*1e-7*ListOfMaterials[i].ElementsConc[k]*ListOfMaterials[i].Density);
	    ListOfMaterials[i].StoppingZEdpass[j][ie]+=(dEdX.d[ie]*1e-15*1e-7*ListOfMaterials[i].ElementsConc[k]*ListOfMaterials[i].Density);}


	}  /* End of loop through elements of target material */
      }/* End of loop through actual  projectile elements */
    } /* End of loop through possible projectile elements */
  } /* End of loop through target materials */
...
int PrepareStoppingTablesfile(){
  ...
  FILE *felstop = fopen("./elstop.in", "rb");
  int NL;    
  
  /* Go through materials and create stopping tables for all existing elements */
  for(i=0;i<NumberOfMaterials;i++){
    ListOfMaterials[i].StoppingZEfile=(float**)malloc(sizeof(float*)*MAX_ELEMENT_NO); /* pointers to stopping arrays */
ListOfMaterials[i].StoppingNLfile = calloc(MAX_ELEMENT_NO, sizeof(int*));
    ListOfMaterials[i].Efile=(float**)malloc(sizeof(float*)*MAX_ELEMENT_NO); /* pointers to stopping arrays */
    if(ListOfMaterials[i].StoppingZEfile==NULL){return -1;}
    if(ListOfMaterials[i].Efile==NULL){return -1;}
    /* Go through possible elements which might be slowed down in this materials */
    for(j=0;j<=ZMAXdpass;j++){ /* Goes through all possible projectiles */
      if(existing_elements[j]==1){ /* ok, element might occur, calculate */
	
	if (fscanf(felstop, "%d", &NL) != 1) {
	  printf("Erreur lecture NL\n");
	  fclose(felstop);
	  return 1;
	}
	
	ListOfMaterials[i].StoppingZEfile[j]=(float*)calloc(NL,sizeof(float));
	ListOfMaterials[i].Efile[j]=(float*)calloc(NL,sizeof(float));
	if(ListOfMaterials[i].StoppingZEfile[j]==NULL){return -1;}
	ListOfMaterials[i].StoppingNLfile[j]=NL ;

	for (int ie = 0; ie < NL; ie++) {
	  if (fscanf(felstop, "%f %f", &	ListOfMaterials[i].Efile[j][ie],&   ListOfMaterials[i].StoppingZEfile[j][ie]) != 2) {
	...
	for (int ie = 0; ie < NL; ie++) {
	  printf("%f %f\n", ListOfMaterials[i].Efile[j][ie],&   ListOfMaterials[i].StoppingZEfile[j][ie]);
	}
...
int print_stopping_table(int ionZ, double ionM, int target, double e_min, double e_max, double e_step){
  ...
  FILE* elec_stopping_fp2 ;
  energy=e_min;
  printf("ENERGI in %d %f %f \n",ionZ,e_max, ionM);
  ...
  elec_stopping_fp=OpenFileContinuous(OutputFileBaseName,".elec_loss");
  elec_stopping_fp2=OpenFileContinuous(OutputFileBaseName,".elec_loss2");
  if(elec_stopping_fp==NULL){
    message_error(-1002,"Cannot open elec_loss file !\n");
    return -1002;
  }
  if (no_headers_in_files==0) {
    fprintf(elec_stopping_fp,"#NAME: %s \n", OutputFileBaseName,".elec_loss");
    fprintf(elec_stopping_fp,"#TITLE: Electronic Stopping Power \n");
    fprintf(elec_stopping_fp,"#DATE: %s", asctime(timeinfo) );
    fprintf(elec_stopping_fp,"#COLUMN_NAMES: E | N1 | N2 | Eloss  \n");
    fprintf(elec_stopping_fp,"#COLUMN_TITLES: Energy | N3 | N4| Stopping power \n");
    fprintf(elec_stopping_fp,"#COLUMN_UNITS: eV | N5 | N6 | ev/nm \n");
    fprintf(elec_stopping_fp,"#COLUMN_FACTOR: %i | %i | %i  | 1.0 \n", 1,1,1);
    fprintf(elec_stopping_fp," \n");
    //	fclose(elecfp);
  }
    
  int N = 100;

  float r = powf(e_max / e_min, 1.0f / (N - 1));


  while(energy<=e_max){
    switch (stopping_model){
    case(0):
      stopping   = (1 * 10.0) * ElectronicStopping(ionZ,ionM,energy,target); /* factor 10, because flightpath is in nm */
      break;
    case(1) :
      //      printf("EnA in %d %f %f %d \n",ionZ,energy, ionM,target);
      stopping   = (1 * 1.) * ElectronicStoppingdpass(ionZ,ionM, energy,target); /* factor 1, because flightpath is in nm */
      break;
    case(2) :
      stopping=ElectronicStoppingfile(ionZ, energy,target); /* factor 1, because flightpath is in nm */
      break;
    }
    ...
    printf("%g\t%g\n",energy,stopping);
    fprintf(elec_stopping_fp,"%g\t 0\t 0\t %g\n",energy,stopping);
    fprintf(elec_stopping_fp2,"%g\t %g\n",energy,stopping);
